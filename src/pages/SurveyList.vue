...
export default {
  ...
  methods: {
    async search(condition) {
                  ...
                  const param = {"classId":this.condition.class,"studentNos":this.condition.student_Nos,"startDateStr":this.condition.startDateStr,"endDateStr":this.condition.endDateStr,"pageNo":1,"pageSize":50} //"status":this.condition.status?this.condition.status:0,
                  const surveys = await this.$http.post(`intern/student/intern/survey/list`,param);
                  const surveys_data = [];
                  _.each(surveys.body,(survey,index)=>{
                      surveys_data.push([{'text':survey.surveryTime},{'text':survey.enterpriseName},{'text': '查看',"link": "/survey/detail?id="+survey.surveryId}])
                  })
